...
public class MInvoice extends X_C_Invoice implements DocAction , DocumentReversalEnabled
{
	...
	public String completeIt()
	{
		...
		if (PAYMENTRULE_Cash.equals(getPaymentRule()) && !fromPOS )
		{
            ...
            //mfrojas 20201127 se agrega ofbforward.
            //Si no esta el sysconf, se sigue normal. 
            //Si está, el diario a elegir es distinto.
            
            if(!OFBForward.HFBCCashJournal())
            {
            	if (posId != 0)
            	{
			...
			{
				//mfrojas elegir diario de la persona conectada.
				int cashb = DB.getSQLValue(get_TrxName(), "SELECT coalesce(max(c_Cashbook_id),0) from c_cashbook "
						+ " WHERE ad_user_id = "+Env.getAD_User_ID(getCtx()));
				if(cashb > 0)
				{
					//Si es mayor que cero, entonces existe un cashbook del usuario conectado.
					//Revisar si existe un cash no completo de la persona conectada.
					int cashid = DB.getSQLValue(get_TrxName(), "SELECT coalesce(max(c_cash_id),0) from c_Cash "
							+ " WHERE c_Cashbook_id = "+cashb+" AND docstatus like 'DR' and isactive='Y'");
					if(cashid > 0)
					{
						//Si es mayor que cero, existe un diario de efectivo abierto. 
						cash = new MCash(getCtx(), cashid, get_TrxName());

					}
					else
					{
						//Si no existe, se debe crear un nuevo diario de efectivo
						cash = new MCash(getCtx(), 0, get_TrxName());
						cash.setC_CashBook_ID(cashb);
						cash.setStatementDate(this.getDateInvoiced());
						cash.setDateAcct(this.getDateAcct());
						cash.save();
				...
				{
					throw new AdempiereException ("No existe un libro efectivo para el usuario conectado.");
				}
