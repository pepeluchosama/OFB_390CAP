...
public class MInOut extends X_M_InOut implements DocAction , DocumentReversalEnabled
{
	...
	{
		//	Warehouse Org
		//mfrojas 20210706 Se comenta lineas para que no valide si una org corresponde o no al almacen
/*		if (newRecord)
		{
			MWarehouse warehouse = MWarehouse.get(getCtx(), getM_Warehouse_ID());
			if (warehouse.getAD_Org_ID() != getAD_Org_ID())
...
*/
	...
	public MInOut reverseIt(boolean isAccrual)
	{
		...
		//mfrojas 20220803
		//Si existe un tipo de documento especifico para anulaciones, se utiliza. 
		//Si no, es el mismo tipo del original
		...
		String sql = "SELECT coalesce(max(c_doctype_id),0) from c_doctype where lower(name) like "
				+ " '%reversa de%' and docbasetype like 'MMS' and isactive='Y'";
		int val = DB.getSQLValue(get_TrxName(), sql);
		
		if(isSOTrx() && val > 0)
			reversal = copyFrom (this, reversalMovementDate, reversalDate,
					val, isSOTrx(), false, true, get_TrxName(), true);
		else
			reversal = copyFrom (this, reversalMovementDate, reversalDate,
					getC_DocType_ID(), isSOTrx(), false, true, get_TrxName(), true);
		if (reversal == null)
