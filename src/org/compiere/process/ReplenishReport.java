...
public class ReplenishReport extends SvrProcess
{
	...
	/*	//mfrojas 20210401
		//Se agrega 2 campos. qtypendiente y ocnumber para hfbc
		
		sql = "UPDATE T_Replenish SET qtypendiente = (SELECT coalesce(sum(qtyreserved),0) "
				+ " FROM c_orderline WHERE isactive='Y' AND m_product_id = T_Replenish.m_product_id and c_order_id in "
				+ "(SELECT c_order_id from c_order where docstatus not in ('DR','VO') AND isactive='Y'))";
		
		no = DB.executeUpdate(sql, get_TrxName());
		if(no != 0)
			log.fine("Updated qtypendiente = "+ no);
		
		sql = "SELECT m_product_id from t_replenish";
		...
		try
		{
			...
			{
				ocnumber = null;
				product_id = rs.getInt("m_product_id");
				sql2 = "SELECT documentno FROM c_order where isactive='Y' and docstatus not in ('DR','VO') "
						+ " and c_order_Id in (SELECT c_order_id "
						+ " from c_orderline where isactive='Y' and m_product_id = "+product_id+")";
				pstmt2 = DB.prepareStatement(sql2, get_TrxName());
				rs2 = pstmt2.executeQuery();
				while(rs2.next())
				{
					if(ocnumber.compareTo("") == 0)
					{
						ocnumber = rs2.getString("documentno");
					}
					else
						ocnumber = ocnumber.concat(" , ").concat(rs2.getString("documentno"));
				}
			}
			rs.close ();
			pstmt.close ();
